...
class CTP515(CatPhanModule):
    ...
    #v3 modificación: Tres filas de ROIs con offsets de 0º, +120º y 240º
    roi_settings = {
        #V3 Fila 1 ángulos originales
        "15-1": {
            "angle": roi_angles[0],
            "distance": roi_dist_mm,
        ...
        #V3 Fila 2 +120º
        "15-2": {
            "angle": roi_angles[0] + 120,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[0],
        },
        "9-2": {
            "angle": roi_angles[1] + 120,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[1],
        },
        "8-2": {
            "angle": roi_angles[2] + 120,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[2],
        },
        "7-2": {
            "angle": roi_angles[3] + 120,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[3],
        },
        "6-2": {
            "angle": roi_angles[4] + 120,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[4],
        },
        "5-2": {
            "angle": roi_angles[5] + 120,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[5],
        },
        ...
        # FILA 3 - Ángulos + 240º
        "15-3": {
            "angle": roi_angles[0] + 240,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[0],
        },
        "9-3": {
            "angle": roi_angles[1] + 240,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[1],
        },
        "8-3": {
            "angle": roi_angles[2] + 240,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[2],
        },
        "7-3": {
            "angle": roi_angles[3] + 240,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[3],
        },
        "6-3": {
            "angle": roi_angles[4] + 240,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[4],
        },
        "5-3": {
            "angle": roi_angles[5] + 240,
            "distance": roi_dist_mm,
            "radius": roi_radius_mm[5],
        },
...
class CatPhanBase(ResultsDataMixin[CatphanResult], QuaacMixin):
    ...
    #V1 añadimos la propiedad linac_name
    @property
    def linac_name(self) -> str:
        """Return the linac name from the DICOM tag (3002, 0020) Radiation Machine Name."""
        try:
            # Intentar obtener el nombre del acelerador del stack de imágenes
            if hasattr(self.dicom_stack, 'metadata') and self.dicom_stack.metadata:
                # Intenta con RadiationMachineName
                if hasattr(self.dicom_stack.metadata, 'RadiationMachineName'):
                    return str(self.dicom_stack.metadata.RadiationMachineName)
                # Intenta con StationName
                if hasattr(self.dicom_stack.metadata, 'StationName'):
                    return str(self.dicom_stack.metadata.StationName)
        
            # Si no está en metadata, intentar con el primer slice
            if len(self.dicom_stack) > 0:
                first_image = self.dicom_stack[0]
                if hasattr(first_image, 'metadata') and first_image.metadata:
                    if hasattr(first_image.metadata, 'RadiationMachineName'):
                        return str(first_image.metadata.RadiationMachineName)
                    if hasattr(first_image.metadata, 'StationName'):
                        return str(first_image.metadata.StationName)
    ...
    def publish_pdf(
        ...
        """
        #V1 añadimos metadata con el acelerador
        if metadata is None:
            metadata = {}
