...
class WinstonLutz(ResultsDataMixin[WinstonLutzResult], QuaacMixin):
        ...
        **kwargs,
        ...
        try:
            ...
            with io.BytesIO() as stream:
                ...
                return img
    #V1 añado propiedad para coger el nombre del acelerador. Lo hace para todas, pero valdría con una única imagen
    @property
    def linac_name(self) -> str:
        """Return the linac name from the DICOM tag (3002, 0020) Radiation Machine Name."""
        try:
            # Intentar obtener el nombre del acelerador de las imágenes
            for img in self.images:
                if hasattr(img, 'metadata') and img.metadata:
                    # Intenta con RadiationMachineName
                    if hasattr(img.metadata, 'RadiationMachineName'):
                        return str(img.metadata.RadiationMachineName)
                    # Intenta con StationName
                    if hasattr(img.metadata, 'StationName'):
                        return str(img.metadata.StationName)
            ...
            return "Unknown"
    #V1 fin de la propiedad añadida

    @property
    def beam_energy(self) -> str:
        """Return the beam energy extracted from DICOM tag (3002,0004) RT Image Description.
        Extracts the text that appears before '[MV]' in the description of the first image.
        E.g. '10xFFF [MV], 1200 [MU/min]' => '10xFFF MV'
        """
        try:
            for img in self.images:
                if hasattr(img, 'metadata') and img.metadata:
                    tag = img.metadata.get((0x3002, 0x0004))
                    if tag is not None:
                        description = str(tag.value)
                        # Extract text before '[MV]'
                        if '[MV]' in description:
                            energy = description.split('[MV]')[0].strip().rstrip(',').strip()
                            return f"{energy} MV"
            return "Unknown"
        except Exception as e:
            print(f"Error obteniendo beam energy: {e}")
            return "Unknown"
    #V4 fin propiedad beam_energy
    ...
    def publish_pdf(
        ...
        include_location_plot: bool = True,  #V2 Nuevo parámetro
    ):
        ...
        include_location_plot: bool
            Whether to include the 3D location plot in the PDF. Default is True
        """
        if not self._is_analyzed:
            raise ValueError("The set is not analyzed. Use .analyze() first.")
            
        #V1 modificamos metadata con el nombre del acelerador 
        if metadata is None:
            metadata = {}
        ...
        # V4: Add beam energy subtitle below the main title
        canvas.add_text(
            text=f" {self.beam_energy}",
            location=(10, 27),
            font_size=14,
        )
        ...
        for ax in (
            ...
            if self._contains_axis_images(ax):
                ...
                plt.close('all') #V3 intento de sol de superposicion
        #V2 NUEVO: Agregar página con plot_location
        if include_location_plot:
            canvas.add_new_page()
            canvas.add_text(
                text="3D Isocenter Visualization", 
                location=(10, 25.5), 
                font_size=18
            )
            
            data_location = io.BytesIO()
            self.plot_location(show=False)
            plt.savefig(data_location, bbox_inches='tight', dpi=100)
            plt.close('all')    #V3 introducimos all
            canvas.add_image(
                image_data=data_location, 
                location=(2, 4), 
                dimensions=(18, 18)
            )
        #V2 fin de nueva pg con imagen 3D    
        canvas.finish()
