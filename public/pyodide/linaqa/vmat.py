...
* **dynamic mlc at gantry angle (slinding window) (aka T01 test)
...
class VMATBase(ABC, ResultsDataMixin[VMATResult], QuaacMixin):
    ...
    #V1 agregamos una propiedad para obtener el nombre del acelerador
    @property
    def linac_name(self) -> str:
        """Return the linac name from the DICOM tag (3002, 0020) Radiation Machine Name."""
        try:
            # Intentar obtener el nombre del acelerador de la imagen open
            if hasattr(self.open_image, 'metadata') and self.open_image.metadata:
                #intenta con RadiationMachineName
                if hasattr(self.open_image.metadata, 'RadiationMachineName'):
                    return str(self.open_image.metadata.RadiationMachineName)
                #intenta con StationName
                if hasattr(self.open_image.metadata.StationName):
                    return str(self.open_image.metadata.StationName)
                       
            # Si no está en open_image, intentar con dmlc_image
            if hasattr(self.dmlc_image, 'metadata') and self.dmlc_image.metadata:
                if hasattr(self.dmlc_image.metadata, 'RadiationMachineName'):
                    return str(self.dmlc_image.metadata.RadiationMachineName)
                if hasattr(self.dmlc_image.metadata, 'StationName'):
                    return str(self.dmlc_image.metadata.StationName)
                        
            return "Unknown"
        
        except Exception as e:
            print(f"Error obteniendo linac name: {e}")
            return "Unknown"
    #V1 fin de la nueva propiedad

    #V6 mod informe para D_MLC
    @property
    def open_gantry_angle(self) -> float:
        """Retorna el ángulo de gantry de la imagen open."""
        try:
            if hasattr(self.open_image, 'metadata') and self.open_image.metadata:
                if hasattr(self.open_image.metadata, 'GantryAngle'):
                    return float(self.open_image.metadata.GantryAngle)
            return 0.0
        except Exception as e:
            print(f"Error obteniendo ángulo de gantry open: {e}")
            return 0.0

    @property
    def dmlc_gantry_angle(self) -> float:
        """Retorna el ángulo de gantry de la imagen dmlc."""
        try:
            if hasattr(self.dmlc_image, 'metadata') and self.dmlc_image.metadata:
                if hasattr(self.dmlc_image.metadata, 'GantryAngle'):
                    return float(self.dmlc_image.metadata.GantryAngle)
            return 0.0
        except Exception as e:
            print(f"Error obteniendo ángulo de gantry dmlc: {e}")
            return 0.0
    #V6 Fin mod informe para D_MLC

    @property
    @abstractmethod
    def default_segment_size_mm(self) -> tuple[float, float]:
         return 5, 180
        #V3 calmbio el pass por return default de 5,180 a ver que tal...
        #pass
    ...
    #V3 pruebo a añadir un nuevo método para obtener el tamaño de ROI según el linac
    def _get_segment_size_for_linac(self, segment_size_mm: tuple | None = None) -> tuple[float, float]:
        """Determina el tamaño del segmento según el acelerador.
        Parameters
        ----------
        segment_size_mm : tuple | None
        Tamaño especificado por el usuario. Si es None, usa valores automáticos.
        Returns
        -------
        tuple[float, float]
        El tamaño (width, height) en mm a utilizar.
        """
        if segment_size_mm is not None:
            return segment_size_mm
    
        #V3 Configuraciones específicas por acelerador
        if self.linac_name == "TrueBeam1":
            return (5, 300)
        #V3 Valor por defecto
        return self.default_segment_size_mm
    #V3 fin del nuevo método
    ...
    def analyze(
        ...
        #V2 cambio segment_size_mm: tuple | None = None, por segment_size_mm: tuple = (5, 180),
        ...
        #V3 Obtenemos el tamaño correcto según linac, llamando al método que cree
        segment_size_mm = self._get_segment_size_for_linac(segment_size_mm)  
    ...
    def publish_pdf(
        ...
        """
        #V1 creamos metadata con el nombre del linac
        if metadata is None:
            metadata = {}
        #if 'Linac' not in metadata:
        metadata['Linac'] = self.linac_name

        #V1 Agregar el nombre del físico
        metadata['Physicist'] = 'Ibán Conles Picos'
        #V1 fin de metadata añadido

        #V7 Para análisis DRCS: añadir ángulo de gantry de la imagen DMLC (tag 300A,011E)
        
        if self.__class__.__name__ == 'DRCS':
            try:
                gantry_angle = self.dmlc_gantry_angle

                # Normalizar ángulos al rango [0, 360)
                angle = gantry_angle % 360

                # Normalizar ángulos cercanos a 0°
                if angle <= 0.5 or angle >= 359.5:
                    angle = 0.0

                # Redondeo a entero para la lógica
                angle_rounded = int(round(angle))

                # Caso especial: arco dinámico
                if angle_rounded == 179:
                    metadata['Ángulo de gantry'] = "Arco Dinámico"
                else:
                    metadata['Ángulo de gantry'] = f"{angle_rounded}°"

            except Exception as e:
                print(f"Error obteniendo ángulo de gantry DMLC: {e}")
        #V7 fin
        #V1 aqui imprimí por consola los tag de la imagen para ver si los cogía bien. Lo comento para quitarlo
        #print("=== DICOM Tags disponibles ===")
        #if hasattr(self.open_image, 'metadata'):
            #print("Open image tags:")
            #for attr in dir(self.open_image.metadata):
                #if not attr.startswith('_'):
                    #print(f"  {attr}")
        #print(f"Linac name obtenido: {self.linac_name}")
        ...
        #V6 MODIFICACIÓN: Detectar si es análisis D_MLC para mostrar ángulos o texto estándar
        #is_dmlc_analysis = isinstance(self, D_MLC)
        is_dmlc_analysis = self.__class__.__name__ == 'D_MLC'
    
        if is_dmlc_analysis:
            # Para análisis D_MLC: mostrar ángulos de gantry
            canvas.add_text(
                text=f"Imagen Gantry = {self.open_gantry_angle:.0f}°", 
                location=(4, 22), 
                font_size=18
            )
            canvas.add_text(
                text=f"Imagen Gantry = {self.dmlc_gantry_angle:.0f}°", 
                location=(14, 22), 
                font_size=18
            )
        else:
            #V6 Para otros análisis (DRGS, DRMLC, DRCS): texto estándar
...
#v4 añadimos una clase para hacer análisis SW
class VMATLinearBaseSW(VMATBase, ABC):
    """Class para hacer el análisis SW 
    """
    ...
    def default_segment_size_mm(self) -> tuple[float, float]:
        return 20, 180
    #V3 vuelvo a calmbiar el default. Conflicto con default_segment_size_mm de otra clase???

    def _identify_images(self, image1: DicomImage, image2: DicomImage):
        """Identify which image is the DMLC and which is the G0º image."""
        #v4 obtener gantry
        def get_gantry_angle(image: DicomImage) -> float | None:
            """Extract gantry angle from DICOM metadata."""
            try:
                if hasattr(image, 'metadata') and image.metadata:
                    # Tag (300A, 011E) - Gantry Angle
                    if hasattr(image.metadata, 'GantryAngle'):
                        return float(image.metadata.GantryAngle)
            except Exception as e:
                print(f"Error obtaining gantry angle: {e}")
            return None
    
        def is_zero_angle(angle: float | None, tolerance: float = 1.0) -> bool:
            """Check if angle is approximately 0 degrees (or 360 degrees).
        
            Parameters
            ----------
            angle : float | None
                The gantry angle to check
            tolerance : float
                Tolerance in degrees for considering angle as zero (default 1.0)
        
            Returns
            -------
            bool
                True if angle is close to 0 or 360 degrees
            """
            if angle is None:
                return False
            ...
            # Check if close to 0 or 360
            return normalized_angle <= tolerance or normalized_angle >= (360 - tolerance)
        
        # Get gantry angles
        angle1 = get_gantry_angle(image1)
        angle2 = get_gantry_angle(image2)
    
        # Identify images based on gantry angle
        if is_zero_angle(angle1) and not is_zero_angle(angle2):
            self.open_image = image1
            self.dmlc_image = image2
            #print(f"Identified by gantry angle - Open: {angle1}°, DMLC: {angle2}°")
            return
        elif is_zero_angle(angle2) and not is_zero_angle(angle1):
            self.open_image = image2
            self.dmlc_image = image1
            #print(f"Identified by gantry angle - Open: {angle2}°, DMLC: {angle1}°")
            return
...
class VMATLinearBase(VMATBase, ABC):
    ...
    def default_segment_size_mm(self) -> tuple[float, float]:
        return 5, 180
    #V3 vuelvo a calmbiar el default de 5,100 a 5,180 a ver que tal...
...
#V4 clase de análisis SW

@capture_warnings
class D_MLC(VMATLinearBaseSW):
    _url_suffix = "drgs.zip"
    _result_header = "Slinding Window at Gantry Angle"
    _result_short_header = "SW/GA"
    ...
    def _update_r_corrs(self):
        """Para análisis de una sola ROI, calcular desviación respecto a 100%"""
        if len(self.segments) == 1:
            # Con una sola ROI, comparar directamente con 100%
            segment = self.segments[0]
            segment.r_dev = segment.r_corr - 100

    @staticmethod
    def run_demo():
        """Run the demo for the Dose Rate & Gantry Speed test."""
        vmat = D_MLC.from_demo_images()
        vmat.analyze(segment_size_mm=(20, 180))  #V2 le paso los tamaños del ROI
        print(vmat.results())
        vmat.plot_analyzed_image()

#V4 FIN clase de análisis SW
...
class DRGS(VMATLinearBase):
    ...
    def run_demo():
        ...
        vmat.analyze(segment_size_mm(5, 180))  #V2 le paso los tamaños del ROI
        print(vmat.results())
        vmat.plot_analyzed_image()
...
class DRCS(VMATBase):
    ...
    def run_demo():
        ...
        #V1 aquí puedes modificar el segment_size con  analyze(segment_size_mm(x,y))
        vmat.analyze()
        print(vmat.results())
        vmat.plot_analyzed_image()
