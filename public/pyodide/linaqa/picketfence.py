...
class PFDicomImage(image.LinacDicomImage):
    ...
    #V1 variable?
    radiation_machine_name: str | None

    def __init__(self, path: str, **kwargs):
        crop_mm = kwargs.pop("crop_mm", 3)
        self._central_axis = kwargs.pop("central_axis", None)
        super().__init__(path, **kwargs)
        #V1 Extraer el nombre del acelerador
        try:
            self.radiation_machine_name = self.metadata.get((0x3002, 0x0020), None)
            if self.radiation_machine_name:
                #V1 Limpiar el valor si es necesario (algunos DICOMs tienen espacios extra)
                self.radiation_machine_name = str(self.radiation_machine_name).strip()
                #Quitamos los tags que no necesitamos (3002, 0020) y el nombre del tag
                if "Radiation Machine Name" in self.radiation_machine_name:
                #V1 Extraer solo el nombre después de los dos puntos
                    parts = self.radiation_machine_name.split(":")
                if len(parts) > 1:
                    self.radiation_machine_name = parts[-1].strip().strip("'\"")
        except:
            self.radiation_machine_name = None
...
class PicketFence(ResultsDataMixin[PFResult], QuaacMixin):
    ...
    def publish_pdf(
        ...
        if metadata is None:
            metadata = {}
        #V1 obtener el nombre del acelerador
        if hasattr(self.image, 'radiation_machine_name') and self.image.radiation_machine_name:
            metadata['Linac'] = self.image.radiation_machine_name
